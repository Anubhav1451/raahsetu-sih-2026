...
| GPS fleet tracking and delivery visibility | Secure backend and operator vehicle discovery complete | Migration `004_fleet_tracking.sql` adds vehicle assets, position history and delivery jobs. Authenticated API scopes reads by reviewer region, lists an operator's assigned vehicles and restricts position writes to the assigned operator. A mobile GPS client and delivery creation workflow remain pending. |
