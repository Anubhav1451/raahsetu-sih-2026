...
The default OSM explorer uses Leaflet with viewport-requested OpenStreetMap tiles. All Northeast fits on opening; users can pan/zoom and follow the selected real journey. Risk-A* stays server-side and 3D terrain remains a separate view. Synthetic routes are excluded from the real basemap. Existing state search selects journey endpoints; cross-state routing is not introduced by this map change.
