...
export const createDelivery = (token: string, payload: { vehicle_id: string; region_code: string; commodity: string; origin_name: string; destination_name: string; eta_at?: string | null }) => request<DeliveryJob>("/api/v1/deliveries", { method: "POST", headers: { "Content-Type": "application/json", Authorization: `Bearer ${token}` }, body: JSON.stringify(payload) });
